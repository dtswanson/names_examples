...
def offer_install(module):
    if sg.popup_yes_no(f'The program failed to import a package. You need to install {module}.',
                       'Would you like PySimpleGUI to install this package for you?',
...
def settings_window():
    ...
    layout = [[sg.T('Program Settings', font='DEFAULT 25')],
              ...
              [sg.CB('Check That Imported Modules Are Installed', sg.user_settings_get_entry('-check imports-', False),
                     k='-CHECK IMPORTS-')],
...
def make_window():
    ...
    left_col = sg.Column([
        ...
        [sg.Text('Filter (F1):', tooltip=filter_tooltip, s=8),
        ...
        [sg.Text('Find (F2):', tooltip=find_tooltip, s=8),
         sg.Input(size=(25, 1), enable_events=True, key='-FIND-', tooltip=find_tooltip),
         sg.T(size=(15, 1), k='-FIND NUMBER-')],
        [sg.Text('Path (F3):', tooltip=run_tooltip, s=8),
         sg.Input(size=(25, 1), enable_events=True, key='-RUN PATH-', tooltip=run_tooltip)],
    ], element_justification='l', expand_x=True, expand_y=True)

    lef_col_find_re = sg.pin(sg.Col([
        [sg.Text('Find (F4):', tooltip=find_re_tooltip, s=8),
    ...
    options_at_bottom = sg.pin(sg.Column([[sg.CB('Verbose', enable_events=True, k='-VERBOSE-',
                                           ...
                                           sg.CB('Show ALL file types',
                                                 default=sg.user_settings_get_entry('-show all files-', False),
                                                 enable_events=True, k='-SHOW ALL FILES-'),
                                           ]],
    ...
    window = sg.Window('PSG Demo & Project Browser', layout, finalize=True, resizable=True, use_default_focus=False,
                       right_click_menu=sg.MENU_RIGHT_CLICK_EDITME_VER_EXIT, auto_save_location=True)
...
def pip_install_latest():
    ...
    layout = [[sg.Text('Installing PySimpleGUI', font='_ 14')],
              [sg.Multiline(s=(90, 15), k='-MLINE-', reroute_cprint=True, reroute_stdout=True, echo_stdout_stderr=True,
                            write_only=True, expand_x=True, expand_y=True)],
              [sg.Push(), sg.Button('Downloading...', k='-EXIT-'), sg.Sizegrip()]]

    window = sg.Window('Pip Install PySimpleGUI Utilities', layout, finalize=True, keep_on_top=True, modal=True,
                       disable_close=True, resizable=True)
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED or (event == '-EXIT-' and window['-EXIT-'].ButtonText == 'Done'):
            ...
            sg.cprint('Done Installing PySimpleGUI.  Click Done and the program will restart.', c='white on red',
        ...
        if event == 'Edit':
            ...
            for file in values['-DEMO LIST-']:
                ...
                if line is not None:
                    ...
                    if not get_editor():
                        sg.popup_error_with_traceback('No editor has been configured',
                                                      'You need to configure an editor in order to use this feature',
                                                      'You can configure the editor in the Demo Brower Settings or the PySimpleGUI Global Settings')
                    else:
            ...
            for file in files_to_run:
                ...
                if sg.user_settings_get_entry('-check imports-', False) and not check_imports_in_file(file_to_run):
                    sg.cprint(
                        f'The demo program {os.path.basename(file_to_run)} depends on modules that are not installed.')
                else:
